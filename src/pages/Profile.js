...
export default function Profile() {
  ...
  const [timeZone, settimeZone] = useState(ud.accountInfo.timeZone ?? Intl.DateTimeFormat().resolvedOptions());
  ...
  const handleTimeZoneChange = (event) => {
    axios.patch('timezone', { timeZone: event.target.value }).then((res) => {
      ...
      newUd.accountInfo.timeZone = event.target.value;
          ...
          >
            <FormControl sx={{ m: 1 }}>
              ...
              <Autocomplete
                ...
                sx={{ width: 300, mt: 3 }}
                defaultValue={ud.accountInfo.countryName}
